program Bundy;
{
           TieBundy -  CGI  - Version 0.1/CGI
                            - May 28 2013

       Display a random bundy quote on web page.
...
// If using FPC enable the below by removing the .  If using VP ignore this.
{$mode TP}
uses
  dos;                          {Disk}
...
var
  f: Text;
begin
  Assign(F, exedir + 'bundy.cfg');           {tiequote.cfg is in the same dir as the tieread.exe}
  {$I-}Reset(f);{$I+}
  if ioresult<>0 then                           {File not found!}
  begin
    WriteLn('<HTML>');
    WriteLn('The Config file does not exist.<BR>');
    WriteLn('Please place the bundy.cfg in your CGI-BIN Directory.<BR>');
    WriteLn('<BR> <BR>');
    WriteLn('<BR>');
    WriteLn('<BR><BR>');
    WriteLn('</BODY> </HTML>');
...
begin
  ...
  TieVersion := '<Font Color="grey">TieBundy v0.1</FONT>';
